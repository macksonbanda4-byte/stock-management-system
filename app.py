...
def notify_users(action_message):
    ...
    for _, row in users.iterrows():
        if "phone_number" in users.columns and row.get("phone_number"):
            send_sms(row["phone_number"], action_message)
        if "email" in users.columns and row.get("email"):
...
def dashboard_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if not low_stock.empty:
        ...
        if available_cols:
            ...
            st.info("Low stock items detected, but required columns are missing.")
        ...
        if match.empty:
            ...
            if st.button("Issue Stock (Dashboard)"):
                if qty_issue > item["Available Stock"]:
                    ...
                    df.at[idx,"Stock Status"] = "Low Stock" if df.at[idx,"Available Stock"] <= df.at[idx,"Reorder Level"] else "In Stock"
                    save_stock(df)
                    sale = {
                    ...
                    save_sales(sales_df)
                    push_undo("issue_stock", {"stock_index": int(idx), "before_row": before_row, "sale_row": sale})
                    log_activity(st.session_state["username"],"issue_stock_dashboard",f"Issued {qty_issue} of {item['Item']} via dashboard")
                    if df.at[idx,"Available Stock"] <= df.at[idx,"Reorder Level"]:
                        notify_users(f"Low stock alert: {item['Item']} at {item['Location']} (Available: {df.at[idx,'Available Stock']})")
...
def add_item_page(df: pd.DataFrame):
    ...
    initial_stock = st.number_input("Initial Stock Quantity", min_value=0, step=1, value=0)
    if st.button("Add Item"):
        ...
        stock_status = "Low Stock" if initial_stock <= reorder_level else "In Stock"
        new_row = {
            "Category": category,"Item": item_name,"Item Code": item_code,"Brand": brand,"Location": location,
            "Supplier": supplier,"Available Stock": initial_stock,"Reorder Level": reorder_level,
            "Cost Price": cost_price,"Price": price,"Total Value": total_value,"Stock Status": stock_status
        }
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        save_stock(df)
        push_undo("add_item", {"item_code": item_code})
        log_activity(st.session_state["username"],"add_item",f"Added item {item_name} ({item_code})")
        if initial_stock <= reorder_level:
            notify_users(f"New item added with low stock: {item_name} at {location} (Stock: {initial_stock})")
...
def receive_stock_page(df: pd.DataFrame):
    ...
    row = df.loc[idx]
    st.write(f"**Selected:** {row['Item']} ({row['Item Code']}) at {row.get('Location','N/A')}")
    ...
    if st.button("Receive"):
        ...
        df.at[idx,"Stock Status"] = "Low Stock" if df.at[idx,"Available Stock"] <= df.at[idx,"Reorder Level"] else "In Stock"
        save_stock(df)
        push_undo("receive_stock", {"stock_index": int(idx), "before_row": before_row})
        log_activity(st.session_state["username"],"receive_stock",f"Received {qty} of {row['Item']} ({row['Item Code']})")
...
def issue_stock_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if st.button("Issue"):
        ...
        save_stock(df)
        sale = {
            "Item": row["Item"],"Item Code": row["Item Code"],"Location": row.get("Location","N/A"),
            "Quantity Sold": qty,"Selling Price": row["Price"],"Cost Price": row["Cost Price"],
            "Total Sale": qty * row["Price"],"Total Cost": qty * row["Cost Price"],
        ...
        save_sales(sales_df)
        push_undo("issue_stock", {"stock_index": int(idx), "before_row": before_row, "sale_row": sale})
        log_activity(st.session_state["username"],"issue_stock",f"Issued {qty} of {row['Item']} ({row['Item Code']})")
        if df.at[idx,"Available Stock"] <= df.at[idx,"Reorder Level"]:
            notify_users(f"Low stock alert: {row['Item']} at {row.get('Location','N/A')} (Available: {df.at[idx,'Available Stock']})")
...
def edit_item_page(df: pd.DataFrame):
    ...
    reorder_level = st.number_input("Reorder Level", min_value=0, step=1, value=int(row["Reorder Level"]))
    cost_price = st.number_input("Cost Price", min_value=0.0, step=0.01, value=float(row["Cost Price"]))
    price = st.number_input("Selling Price", min_value=0.0, step=0.01, value=float(row["Price"]))
    available_stock = st.number_input("Available Stock", min_value=0, step=1, value=int(row["Available Stock"]))
    if st.button("Save Changes"):
        ...
        df.at[idx,"Stock Status"] = "Low Stock" if available_stock <= reorder_level else "In Stock"
        save_stock(df)
        push_undo("edit_item", {"stock_index": int(idx), "before_row": before_row})
        log_activity(st.session_state["username"],"edit_item",f"Edited item {before_row['Item']} ({before_row['Item Code']})")
...
def delete_item_page(df: pd.DataFrame):
    ...
    idx = item_list[item_list == choice].index[0]
    row = df.loc[idx]
    st.warning(f"You are about to delete: {row['Item']} ({row['Item Code']}) at {row.get('Location','N/A')}")
    if st.button("Confirm Delete"):
        deleted_row = row.to_dict()
        df = df.drop(index=idx).reset_index(drop=True)
        save_stock(df)
        push_undo("delete_item", {"deleted_row": deleted_row})
        log_activity(st.session_state["username"],"delete_item",f"Deleted item {deleted_row['Item']} ({deleted_row['Item Code']})")
...
def undo_last_action(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if st.button("Undo"):
        ...
        if a_type == "add_item":
            ...
            save_stock(df)
            log_activity(st.session_state["username"],"undo_add_item",f"Undid add item {code}")
            ...
            for k, v in before_row.items():
                df.at[idx, k] = v
            save_stock(df)
            log_activity(st.session_state["username"],"undo_receive_stock",f"Restored stock row index {idx}")
            ...
            save_sales(sales_df)
            log_activity(st.session_state["username"],"undo_issue_stock",f"Restored stock and removed sale for {sale_row['Item']}")
            ...
            for k, v in before_row.items():
                df.at[idx, k] = v
            save_stock(df)
            log_activity(st.session_state["username"],"undo_edit_item",f"Restored item {before_row['Item']} ({before_row['Item Code']})")
            ...
            save_stock(df)
            log_activity(st.session_state["username"],"undo_delete_item",f"Restored deleted item {deleted_row['Item']} ({deleted_row['Item Code']})")
            st.success("Undo successful: item restored.")
        else:
...
def import_export_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if uploaded_sales:
        ...
        new_sales = normalize_columns(new_sales)
    ...
    if restore_file:
        ...
        restored = normalize_columns(restored)
        if "Item" in restored.columns and "Available Stock" in restored.columns:
            save_stock(restored)
