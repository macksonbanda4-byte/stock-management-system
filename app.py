...
BACKUP_DIR = "backups"
TEST_MODE = True  # SMS/Email simulated
...
def load_stock() -> pd.DataFrame:
    """
    Your CSV header is:
    CATEGORY,Item,ITEM CODE,BRAND,AVAILABLE STOCK,REORDER LEVEL,Unit Price,Total Value,STOCK STATUS

    We map that into the app's internal schema:
    Category,Item,Item Code,Brand,Location,Supplier,Available Stock,Reorder Level,
    ...
    # Map your existing columns to internal names if present
    rename_map = {}
    ...
    if "ITEM CODE" in df.columns:
        rename_map["ITEM CODE"] = "Item Code"
    if "BRAND" in df.columns:
        rename_map["BRAND"] = "Brand"
    if "AVAILABLE STOCK" in df.columns:
        rename_map["AVAILABLE STOCK"] = "Available Stock"
    if "REORDER LEVEL" in df.columns:
        rename_map["REORDER LEVEL"] = "Reorder Level"
    if "Unit Price" in df.columns:
        rename_map["Unit Price"] = "Price"
    if "STOCK STATUS" in df.columns:
        rename_map["STOCK STATUS"] = "Stock Status"
    ...
    for col in expected_cols:
        if col not in df.columns:
            if col in ["Available Stock", "Reorder Level"]:
                df[col] = 0
            elif col in ["Cost Price", "Price", "Total Value"]:
                df[col] = 0.0
            elif col == "Stock Status":
                df[col] = ""
            else:
                df[col] = ""

    # If Cost Price is missing or zero but Price exists, use Price as Cost Price baseline
    df["Cost Price"] = df["Cost Price"].fillna(0)
    df["Price"] = df["Price"].fillna(0)
    df.loc[df["Cost Price"] == 0, "Cost Price"] = df.loc[df["Cost Price"] == 0, "Price"]

    # Recompute Total Value if needed
    df["Available Stock"] = pd.to_numeric(df["Available Stock"], errors="coerce").fillna(
        0
    )
    df["Cost Price"] = pd.to_numeric(df["Cost Price"], errors="coerce").fillna(0.0)
    df["Total Value"] = df["Available Stock"] * df["Cost Price"]

    # Stock status if empty
    df["Stock Status"] = df["Stock Status"].replace("", "In Stock")
    df.loc[df["Available Stock"] <= df["Reorder Level"], "Stock Status"] = "Low Stock"
    df.loc[df["Available Stock"] == 0, "Stock Status"] = "Stock Out"
...
def dashboard_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if scan_code:
        ...
        if match.empty:
            ...
            item = match.iloc[0]
            st.success(
                f"Item Found: {item['Item']} ({item.get('Location','N/A')})"
            )
            ...
            if st.button("Issue Stock (Dashboard)"):
                if qty_issue > item["Available Stock"]:
                    ...
                    if df.at[idx, "Available Stock"] <= df.at[idx, "Reorder Level"]:
                        notify_users(
                            f"Low stock alert: {item['Item']} at {item.get('Location','N/A')} (Available: {df.at[idx,'Available Stock']})"
                        )
...
def add_item_page(df: pd.DataFrame):
    ...
    if st.button("Add Item"):
        ...
        if initial_stock <= reorder_level:
            notify_users(
                f"New item added with low stock: {item_name} at {location or 'N/A'} (Stock: {initial_stock})"
            )
...
def receive_stock_page(df: pd.DataFrame):
    ...
    st.write(
        f"**Selected:** {row['Item']} ({row['Item Code']}) at {row.get('Location','N/A')}"
    )
...
def issue_stock_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    st.write(
        f"Available: {row['Available Stock']} | Reorder Level: {row['Reorder Level']}"
    )
    ...
    if st.button("Issue"):
        ...
        if df.at[idx, "Available Stock"] <= df.at[idx, "Reorder Level"]:
            notify_users(
                f"Low stock alert: {row['Item']} at {row.get('Location','N/A')} (Available: {df.at[idx,'Available Stock']})"
            )
...
def delete_item_page(df: pd.DataFrame):
    ...
    st.warning(
        f"You are about to delete: {row['Item']} ({row['Item Code']}) at {row.get('Location','N/A')}"
    )
...
def import_export_page(df: pd.DataFrame, sales_df: pd.DataFrame):
    ...
    if restore_file:
        ...
        if "Item" in restored.columns and "Available Stock" in restored.columns:
            restored = load_stock()  # re-map schema if needed
            save_stock(restored)
