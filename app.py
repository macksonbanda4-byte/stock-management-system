...
def user_management_page(current_user, current_role):
    ...
    if st.button("Reset Password"):
        if not reset_pass:
            ...
            # ============================================================
# LOAD & SAVE STOCK (auto‑restore backup)
# ============================================================
@st.cache_data
def load_stock():
    backup_path = Path("backups/stock_backup.csv")
    if backup_path.exists():
        df = pd.read_csv(backup_path)
        st.info(f"Loaded stock from backup ({backup_path}).")
    elif os.path.exists(STOCK_FILE):
        df = pd.read_csv(STOCK_FILE)
        st.info("Loaded stock from main file.")
    else:
        df = pd.DataFrame(columns=REQUIRED_STOCK_COLS)
        st.warning("No stock file found — starting fresh.")
...
def save_stock(df):
    ...
    df.to_csv("backups/stock_backup.csv", index=False)
    load_stock.clear()
    st.toast("Stock saved and backup updated ✅")


# ============================================================
# LOAD & SAVE SALES (auto‑restore backup)
# ============================================================
@st.cache_data
def load_sales():
    backup_path = Path("backups/sales_backup.csv")
    if backup_path.exists():
        df = pd.read_csv(backup_path)
        st.info(f"Loaded sales from backup ({backup_path}).")
    elif os.path.exists(SALES_FILE):
        df = pd.read_csv(SALES_FILE)
        st.info("Loaded sales from main file.")
    else:
        cols = ["Date", "Item Code", "Item", "Quantity Sold",
                "Selling Price", "Total", "Customer", "Issued By"]
        df = pd.DataFrame(columns=cols)
        st.warning("No sales file found — starting fresh.")
    return df
...
def save_sales(df):
    ...
    df.to_csv("backups/sales_backup.csv", index=False)
    load_sales.clear()
    st.toast("Sales saved and backup updated ✅")
...
def perform_undo():
    ...
    st.toast("Undo successful — previous state restored 🔄")
    return df_stock, df_sales


# ============================================================
# FIXED ITEM PICKER WITH SEARCH
# ============================================================
...
# ============================================================
# ADD / EDIT / DELETE / RECEIVE / TRANSFER / ISSUE PAGES
# ============================================================
...
def edit_item_page(df_stock, current_user):
    ...
    if st.button("Save Changes"):
        ...
        st.success("Item updated successfully ✏️")
        # ============================================================
# DELIVERY NOTE (TEXT ONLY, NO PDF)
# ============================================================
def generate_issue_text_report(sale):
...
def edit_item_page(df_stock, current_user):
    ...
    if st.button("Confirm Delete"):
        ...
        st.success("Item deleted successfully 🗑️")
...
def issue_stock_page(df_stock, df_sales, current_user):
    ...
    if st.button("Issue Stock"):
        ...
        st.text(text_report)
        st.download_button("Download Delivery Note (Text)",
                           text_report,
...
def backup_system():
    ...
    if st.button("Create Backup Now"):
        ...
        st.success(f"Backup created successfully: {backup_folder}")
        st.toast("Backup completed ✅")
...
def restore_system():
    ...
    if st.button("Restore Selected Backup"):
        ...
        load_stock.clear()
        load_sales.clear()
        st.success("System restored successfully ♻️")
        st.toast("Restore completed ✅")
...
def main():
    ...
    st.sidebar.success("Data restored from backup automatically ✅")
    ...
    if menu == "Dashboard":
        st.title("📊 Dashboard")
        st.write("Welcome to ID Solar Solutions Stock Management System.")
        st.info("Use the sidebar to navigate between operations, reports, and tools.")

    elif menu == "Stock Operations":
        sub_menu = st.sidebar.selectbox(
            "Select Operation",
            ["Add Item", "Edit Item", "Delete Item", "Receive Stock", "Transfer Stock", "Issue Stock"]
        )
        if sub_menu == "Add Item":
            add_item_page(df_stock, st.session_state["username"])
        elif sub_menu == "Edit Item":
            edit_item_page(df_stock, st.session_state["username"])
        elif sub_menu == "Delete Item":
            delete_item_page(df_stock, st.session_state["username"])
        elif sub_menu == "Receive Stock":
            receive_stock_page(df_stock, st.session_state["username"])
        elif sub_menu == "Transfer Stock":
            transfer_stock_page(df_stock, st.session_state["username"])
        elif sub_menu == "Issue Stock":
            issue_stock_page(df_stock, df_sales, st.session_state["username"])

    elif menu == "Reports":
        sub_menu = st.sidebar.selectbox(
            "Select Report",
            ["Low Stock Report", "Stock Summary", "Sales Report"]
        )
        if sub_menu == "Low Stock Report":
            low_stock_report(df_stock)
        elif sub_menu == "Stock Summary":
            stock_summary_report(df_stock)
        elif sub_menu == "Sales Report":
            sales_report(df_sales)

    elif menu == "System Tools":
        sub_menu = st.sidebar.selectbox(
            "Select Tool",
            ["Backup System", "Restore System", "Import Stock", "Export Stock", "Activity Log"]
        )
        if sub_menu == "Backup System":
            backup_system()
        elif sub_menu == "Restore System":
            restore_system()
        elif sub_menu == "Import Stock":
            import_stock(df_stock, st.session_state["username"])
        elif sub_menu == "Export Stock":
            export_stock(df_stock)
        elif sub_menu == "Activity Log":
            activity_log_page()

    elif menu == "User Management":
        user_management_page(st.session_state["username"], st.session_state["role"])
